...
import { back_arrow_icon, shopping_basket } from '../constants/icons';

const namespaceKey = "@@KapaAutoFollow:";
...
const getData = async (key) => {
  try {
    return await AsyncStorage.getItem(namespaceKey + key);
  } catch(e) {
    // error reading value
  }
}
...
const Home = ({ navigation }) => {
  ...
  function renderCategoryData() {
    ...
    async function getFollowing(){
      var following = await getData("following");
      console.log("following: ", following);

      var storedBooks = following.map((item, index) => {
        return {
          ...newBook,
          ...item,
          bookName: item.title,
          id: item.href,
          pageNo: item.totalEpisodes,
          url: item.href,
    ...
    function removeBook(id){
      var newBooks = books.filter(item => item.id != id);
      setBooks([...newBooks]);
      storeData("following", newBooks);
    }
            ...
            onPress={() =>
              {
                ...
                //   book: item,
                // })
                // Linking.openURL(item.url, '_blank');
                if (window) {
